...
import { FiMoreVertical, FiFilter, FiDownload, FiPlus, FiEye, FiEdit2, FiTrash2, FiZap, FiToggleLeft } from "react-icons/fi";
...
const FeatureToggle = ({ name, enabled, onToggle }) => (
  <div className="flex items-center justify-between px-3 py-2.5 rounded-lg bg-gray-50">
    <span className={`text-sm ${enabled ? "text-green-700 font-medium" : "text-gray-600"}`}>{name}</span>
    <button
      type="button"
      onClick={onToggle}
...
// Card 3-dot dropdown
// ─────────────────────────────────────────────
const CardDropdown = ({ plan, onView, onEdit, onStatus, onHighlight, onDelete }) => {
  const [open, setOpen] = useState(false);

  const items = [
    { label: "View",      icon: <FiEye size={14} />,        fn: () => { onView(plan.id);      setOpen(false); } },
    { label: "Edit",      icon: <FiEdit2 size={14} />,      fn: () => { onEdit(plan.id);      setOpen(false); } },
    { label: "Status",    icon: <FiToggleLeft size={14} />, fn: () => { onStatus(plan.id);    setOpen(false); } },
    { label: "Highlight", icon: <FiZap size={14} />,        fn: () => { onHighlight(plan.id); setOpen(false); } },
    { label: "Delete",    icon: <FiTrash2 size={14} />,     fn: () => { onDelete(plan.id);    setOpen(false); }, danger: true },
  ];
...
// Plan Card
// ─────────────────────────────────────────────
const PlanCard = ({ plan, onView, onEdit, onStatus, onHighlight, onDelete }) => {
  ...
  return (
    <div
      className={`relative bg-white rounded-2xl flex flex-col gap-4 p-5 transition-shadow hover:shadow-md ${
        isPopular ? "border-2 border-green-600 shadow-md" : "border border-gray-200"
      }`}
    >
      {isPopular && (
      ...
      <div>
        ...
        <div className="text-2xl font-bold text-green-700 mt-1">
          ...
          <span className="text-sm font-normal text-gray-500"> /{plan.planDuration}</span>
        </div>
      ...
      <ul className="flex flex-col gap-2 flex-1">
        ...
        {featureList.length > 4 && (
          <li className="text-sm text-green-600 font-medium cursor-pointer hover:underline" onClick={() => onView(plan.id)}>
...
const Subscription = () => {
  ...
  const [newSubscription, setNewSubscription] = useState({
    id: "", partnerType: "", planDuration: "", planName: "", totalPrice: "", features: "",
  });
  const [selectedImages, setSelectedImages] = useState({ first: null, second: null, third: null });
  ...
  const syncFeatsFromString = (str) => {
    const set = new Set(str ? str.split(",").map((f) => f.trim()).filter(Boolean) : []);
    setEnabledFeats(set);
  };
  ...
  const toggleFeat = (name) => {
    setEnabledFeats((prev) => {
      ...
      setNewSubscription((s) => ({ ...s, features: Array.from(next).join(",") }));
      return next;
    });
  };
  ...
  const filteredCatalogue = Object.fromEntries(
    Object.entries(FEATURE_CATALOGUE)
      .map(([cat, feats]) => [cat, feats.filter((f) => f.toLowerCase().includes(featSearch.toLowerCase()))])
      .filter(([, feats]) => feats.length > 0)
  );

  const resetForm = () => {
    setNewSubscription({ id: "", partnerType: "", planDuration: "", planName: "", totalPrice: "", features: "" });
  ...
  const handleImageChange = (event, key) => {
    ...
    if (!allowed.includes(file.type)) { alert("Only PNG, JPG and JPEG formats are allowed!"); event.target.value = ""; return; }
    if (file.size > 1 * 1024 * 1024) { alert("File size must be less than 1MB!"); event.target.value = ""; return; }
  ...
  // ──────────────────────────────────────────
  // YOUR ORIGINAL API CALLS — untouched
  // ──────────────────────────────────────────
  ...
  const addOrUpdate = async (e) => {
    e.preventDefault();
    if (!newSubscription.partnerType || !newSubscription.planDuration || !newSubscription.planName || !newSubscription.totalPrice || !newSubscription.features) {
    ...
    try {
      ...
      if (selectedImages.first)  formData.append("firstImage",  selectedImages.first);
      if (selectedImages.second) formData.append("secondImage", selectedImages.second);
      if (selectedImages.third)  formData.append("thirdImage",  selectedImages.third);
      const response = await fetch(`${URI}/admin/subscription/pricing/${endpoint}`, { method, credentials: "include", body: formData });
      const data = await response.json();
  ...
  const edit = async (id) => {
    try {
      ...
      setBannerPreview(data.firstImage ? URI + data.firstImage : null);
      setPage("edit");
    } catch (err) { console.error("Error fetching :", err); }
  };
  ...
  const highlight = async (id) => {
    if (!window.confirm("Are you sure you want to Highlight this Subscription Plan?")) return;
    try {
      const response = await fetch(URI + `/admin/subscription/pricing/highlight/${id}`, { method: "PUT", credentials: "include" });
      const data = await response.json();
      if (response.ok) alert(`Success: ${data.message}`); else alert(`Error: ${data.message}`);
      fetchData();
    } catch (error) { console.error("Error Highlighting :", error); }
  };

  const status = async (id) => {
    if (!window.confirm("Are you sure you want to change this Subscription status?")) return;
    try {
      const response = await fetch(URI + `/admin/subscription/pricing/status/${id}`, { method: "PUT", credentials: "include" });
      const data = await response.json();
      if (response.ok) alert(`Success: ${data.message}`); else alert(`Error: ${data.message}`);
      fetchData();
  ...
  const del = async (id) => {
    ...
    try {
      const response = await fetch(URI + `/admin/subscription/pricing/delete/${id}`, { method: "DELETE", credentials: "include" });
      const data = await response.json();
      if (response.ok) { alert("Subscription deleted successfully!"); fetchData(); }
  ...
  // ── table columns — icons instead of dropdown ──
  const customStyles = {
    rows: { style: { padding: "6px 0", fontSize: "13px", fontWeight: 500, color: "#111827" } },
    headCells: { style: { fontSize: "11px", fontWeight: "600", backgroundColor: "#F9FAFB", color: "#6B7280", textTransform: "uppercase", letterSpacing: "0.05em" } },
  ...
  const columns = [
    {
      ...
      cell: (row, index) => (
        <div className="relative group flex items-center w-full">
          <span className={`min-w-6 flex items-center justify-center px-2 py-1 rounded-md cursor-pointer text-xs font-medium ${row.status === "Active" ? "bg-green-50 text-green-700" : "bg-red-50 text-red-500"}`}>
            {index + 1}
          </span>
    ...
    { name: "Partner Type", selector: (row) => row.partnerType, sortable: true, minWidth: "150px" },
    {
      name: "Plan Name",
      cell: (row) => (
        <span className={`font-medium ${row.highlight === "True" ? "text-green-600" : "text-gray-800"}`}>{row.planName}</span>
      ),
      minWidth: "140px",
    },
    { name: "Duration", selector: (row) => row.planDuration, minWidth: "120px" },
    {
      name: "Total Price",
      cell: (row) => <span className="font-semibold text-green-700">{formatINR(parseInt(row.totalPrice))}</span>,
      minWidth: "140px",
    },
    {
      name: "Features",
      cell: (row) => <span className="text-xs text-gray-500 line-clamp-2">{row.features}</span>,
      minWidth: "260px",
    },
    { name: "Status", cell: (row) => <StatusBadge status={row.status} />, minWidth: "100px" },
    {
      name: "Actions",
      cell: (row) => (
      ...
      <div className="w-full min-h-screen bg-gray-50 p-4 md:p-6">
        ...
        <div className="flex items-center gap-3 mb-6">
          <button
            ...
            className="flex items-center gap-2 text-sm text-gray-600 hover:text-gray-900 bg-white border border-gray-200 rounded-xl px-3 py-2 hover:bg-gray-50 transition-colors"
          >
            <IoArrowBack size={16} /> Back
          </button>
          <div>
            <h1 className="text-xl font-semibold text-gray-900">
              {page === "edit" ? "Edit Subscription Plan" : "Add Subscription Plan"}
            </h1>
            <p className="text-sm text-gray-500">
              {page === "edit" ? "Update the plan details below." : "Fill in the details to create a new plan."}
            </p>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="flex flex-col gap-5">
              ...
              <div className="bg-white border border-gray-200 rounded-2xl p-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-xs text-gray-500 mb-1.5 block">Partner Type <span className="text-red-500">*</span></label>
                    <select
                      required
                      ...
                      onChange={(e) => setNewSubscription({ ...newSubscription, partnerType: e.target.value })}
                    >
                  ...
                  <div>
                    <label className="text-xs text-gray-500 mb-1.5 block">Plan Duration <span className="text-red-500">*</span></label>
                    <select
                      required
                      className="w-full text-sm p-2.5 border border-gray-300 rounded-lg bg-white text-gray-800 focus:outline-none focus:ring-2 focus:ring-green-500"
                      value={newSubscription.planDuration}
                      onChange={(e) => setNewSubscription({ ...newSubscription, planDuration: e.target.value })}
                    >
                      <option value="" disabled>Select duration</option>
                      {["1 Month","2 Months","3 Months","4 Months","5 Months","6 Months","7 Months","8 Months","9 Months","10 Months","11 Months","12 Months"].map(
                        (d) => <option key={d} value={d}>{d}</option>
                      )}
                    </select>
                  </div>
                  <div>
                    <label className="text-xs text-gray-500 mb-1.5 block">Plan Name <span className="text-red-500">*</span></label>
                    <input
                      type="text" required placeholder="e.g. Growth Pro"
                      className="w-full text-sm p-2.5 border border-gray-300 rounded-lg bg-white text-gray-800 focus:outline-none focus:ring-2 focus:ring-green-500"
                  ...
                  <div>
                    <label className="text-xs text-gray-500 mb-1.5 block">Total Price (₹) <span className="text-red-500">*</span></label>
                    <input
                      type="number" min={0} required placeholder="0"
                      className="w-full text-sm p-2.5 border border-gray-300 rounded-lg bg-white text-gray-800 focus:outline-none focus:ring-2 focus:ring-green-500"
                      value={newSubscription.totalPrice}
                      onChange={(e) => setNewSubscription({ ...newSubscription, totalPrice: e.target.value })}
                    />
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white border border-gray-200 rounded-2xl p-6">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-sm font-semibold text-gray-800 flex items-center gap-2">
                    ...
                    <span className="text-xs text-gray-400 font-normal">
                      Selected: <span className="text-green-600 font-medium">{enabledFeats.size}</span> / {ALL_FEATURES.length}
                    </span>
                  </h3>
                  <div className="flex gap-3 text-xs">
                    <button type="button" className="text-green-600 hover:underline"
                      onClick={() => { setEnabledFeats(new Set(ALL_FEATURES)); setNewSubscription((s) => ({ ...s, features: ALL_FEATURES.join(",") })); }}>
                      Select All
                    </button>
                    <span className="text-gray-300">|</span>
                    <button type="button" className="text-gray-400 hover:underline"
                      onClick={() => { setEnabledFeats(new Set()); setNewSubscription((s) => ({ ...s, features: "" })); }}>
                      Clear All
                    </button>
                  </div>
                </div>
                ...
                {enabledFeats.size > 0 && (
                  <div className="flex flex-wrap gap-2 mb-3">
                    {Array.from(enabledFeats).map((f) => (
                      <span key={f} className="inline-flex items-center gap-1 bg-green-50 text-green-700 text-xs px-2.5 py-1 rounded-full">
                        {f.split(" ").slice(0, 2).join(" ")}
                        <button type="button" onClick={() => toggleFeat(f)} className="ml-0.5 text-green-500 hover:text-green-800">×</button>
                      </span>
                    ))}
                  </div>
                )}
                ...
                <div className="relative mb-3">
                  <CiSearch className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={16} />
                  <input
                  ...
                  {Object.entries(filteredCatalogue).map(([cat, feats]) => (
                    <div key={cat}>
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {feats.map((f) => (
                          <FeatureToggle key={f} name={f} enabled={enabledFeats.has(f)} onToggle={() => toggleFeat(f)} />
                        ))}
                      </div>
                    </div>
                  ))}
                ...
                <div className="mt-4">
                  <label className="text-xs text-gray-500 mb-1 block">
                    Or type comma-separated features <span className="text-red-500">*</span>
                  </label>
                  <textarea
                    rows={2} required placeholder="Feature A, Feature B"
                    ...
                    onChange={(e) => { setNewSubscription({ ...newSubscription, features: e.target.value }); syncFeatsFromString(e.target.value); }}
                  />
                </div>
              </div>

              {/* Banner Images — your original 3-image upload */}
              <div className="bg-white border border-gray-200 rounded-2xl p-6">
                <h3 className="text-sm font-semibold text-gray-800 mb-5">🖼 Banner Images</h3>
                <div className="flex flex-col gap-5">
                  {[
                    { key: "first",  label: "First Banner Image",  id: "firstImageUpload"  },
                    { key: "second", label: "Second Banner Image", id: "secondImageUpload" },
                    { key: "third",  label: "Third Banner Image",  id: "thirdImageUpload"  },
                  ].map(({ key, label, id }) => (
                    <div key={key}>
                      <label className="text-xs text-gray-500 mb-2 block">{label}</label>
                      {/* Existing DB image */}
                      {newSubscription?.[`${key}Image`] && (
                        <img src={URI + newSubscription[`${key}Image`]} alt="existing"
                      ...
                      <label htmlFor={id}
                        ...
                        <span className="px-4 py-2.5 text-sm text-gray-400">Upload Image</span>
                        <div className="flex items-center justify-center px-5 py-2.5 bg-gray-800 text-white text-sm">Browse</div>
                        <input type="file" accept="image/png, image/jpeg, image/jpg"
                          onChange={(e) => handleImageChange(e, key)} className="hidden" id={id} />
                      </label>
                      {selectedImages[key] && (
                        <div className="relative mt-2 inline-block">
                          <img src={URL.createObjectURL(selectedImages[key])} alt="preview"
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden sticky top-6">
                ...
                <div className="p-6">
                  <h3 className="text-lg font-semibold text-gray-900">{newSubscription.planName || "Plan Name"}</h3>
                  <div className="text-3xl font-bold text-green-700 mt-1">
                    {newSubscription.totalPrice ? formatINR(Number(newSubscription.totalPrice)) : "₹0"}
                    <span className="text-sm font-normal text-gray-400"> / {newSubscription.planDuration || "Monthly"}</span>
                  </div>
                  <p className="text-xs text-gray-400 uppercase tracking-wide mt-5 mb-3 font-semibold">What's Included</p>
                  <ul className="flex flex-col gap-2">
                    ...
                    {previewFeats.length > 6 && <li className="text-xs text-gray-400">+ {previewFeats.length - 6} more features</li>}
                    {previewFeats.length === 0 && <li className="text-xs text-gray-400 italic">No features selected yet</li>}
                  </ul>
                  <button type="button" className="mt-5 w-full py-3 bg-green-700 text-white text-sm font-medium rounded-xl cursor-default">
                    Get Started Today
                  </button>
                </div>
              </div>

              <div className="bg-purple-50 rounded-xl p-3 flex gap-2 text-xs text-purple-700">
                <span>💡</span>
                <span>Keep plan names descriptive so customers understand the value proposition at a glance.</span>
              </div>

              {/* Sticky footer CTA */}
              <div className="flex justify-end gap-3 pt-4 border-t border-gray-100">
                <Loader />
                <button type="button" onClick={() => { resetForm(); setPage("list"); }}
    ...
    <div className="w-full min-h-screen bg-gray-50 p-4 md:p-6 flex flex-col gap-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Manage and optimise your global subscription tiers and partner configurations.</p>
        </div>
      ...
      {viewMode === "card" && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
          {filteredData.length === 0 ? (
            <div className="col-span-full text-center text-gray-400 py-16 text-sm">No plans found for this partner type.</div>
          ) : (
      ...
      {viewMode === "table" && (
        <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-100">
            <h2 className="text-sm font-semibold text-gray-800">Subscription Pricing List</h2>
          </div>
          <div className="overflow-x-auto">
            <DataTable
              ...
              paginationComponentOptions={{ rowsPerPageText: "Rows per page:", rangeSeparatorText: "of", selectAllRowsItem: true, selectAllRowsItemText: "All" }}
            />
          </div>
        </div>
      )}
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-5">
        <div className="flex justify-between items-center mb-4">
          ...
          <button onClick={() => setViewMode("table")} className="text-sm text-green-600 font-medium hover:underline">View All Records</button>
        </div>
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-100">
              {["Plan Name", "Price", "Partner Type", "Status", "Actions"].map((h) => (
                <th key={h} className="text-left pb-3 text-xs text-gray-400 uppercase tracking-wide font-semibold">{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {datas.slice(0, 5).map((plan) => (
              <tr key={plan.id} className="border-b border-gray-50 last:border-0">
                <td className="py-3 font-medium text-gray-800">{plan.planName}</td>
                <td className="py-3 text-green-700 font-semibold">{formatINR(parseInt(plan.totalPrice))}/mo</td>
                <td className="py-3">
                  <span className="text-xs bg-gray-100 text-gray-500 px-2.5 py-1 rounded-full">{plan.partnerType}</span>
                </td>
                <td className="py-3"><StatusBadge status={plan.status} /></td>
                <td className="py-3">
                  <div className="flex items-center gap-1">
                    <button title="View" onClick={() => view(plan.id)} className="p-1.5 rounded-lg text-gray-400 hover:text-green-600 hover:bg-green-50 transition-colors"><FiEye size={14} /></button>
                    <button title="Edit" onClick={() => edit(plan.id)} className="p-1.5 rounded-lg text-gray-400 hover:text-blue-600 hover:bg-blue-50 transition-colors"><FiEdit2 size={14} /></button>
                    <button title="Delete" onClick={() => del(plan.id)} className="p-1.5 rounded-lg text-gray-400 hover:text-red-600 hover:bg-red-50 transition-colors"><FiTrash2 size={14} /></button>
                  </div>
                </td>
              </tr>
            ))}
      ...
      {/* ════════════════════════════════
          VIEW PLAN MODAL (original logic)
      ════════════════════════════════ */}
      <div className={`${showSubscriptionPlan ? "flex" : "hidden"} fixed inset-0 z-[61] bg-black/50 items-end md:items-center justify-center`}>
        <div className="w-full md:w-[600px] max-h-[80vh] overflow-y-auto bg-white rounded-t-2xl md:rounded-2xl p-6">
          <div className="flex items-center justify-between mb-5">
            ...
            <button onClick={() => setShowSubscriptionPlan(false)} className="text-gray-400 hover:text-gray-600">
              <IoMdClose size={22} />
            </button>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
            {[
              ...
              ["Total Price",  subscriptionPlan.totalPrice ? formatINR(parseInt(subscriptionPlan.totalPrice)) : "—"],
              ["Highlighted",  subscriptionPlan.highlight === "True" ? "Yes" : "No"],
            ].map(([label, val]) => (
              <div key={label}>
          ...
          {/* Banner images — original click-to-open logic */}
          {(subscriptionPlan.firstImage || subscriptionPlan.secondImage || subscriptionPlan.thirdImage) && (
            <div className="mb-5">
                ...
                {["firstImage", "secondImage", "thirdImage"].map((imgKey) =>
                  subscriptionPlan[imgKey] ? (
                    <img key={imgKey}
                      ...
                      onClick={() => window.open(URI + subscriptionPlan[imgKey], "_blank")}
          ...
          <div className="mt-6 flex justify-end">
            <button onClick={() => setShowSubscriptionPlan(false)}
